...
export const Table = <TData,>({
  ...
  /**
   * Column Sizing State (for drag-to-resize)
   */
   ...
   * - Manual columns (size != default 50): use their configured size.
   * - Auto columns (size == default 50): distribute remaining panel width equally.
   * This ensures flex:none cells render at correct sizes immediately,
   * and drag-to-resize works independently for both auto and manual columns.
   */
  useEffect(() => {
    ...
    headers.forEach((h) => {
      ...
      if (size && size !== defaultColumnSizing.size) {
        sizing[h.column.id] = size;
        assignedWidth += size;
      } else {
        autoCount++;
      }
    ...
    const autoWidth = autoCount > 0 ? Math.max((width - assignedWidth) / autoCount, defaultColumnSizing.size) : 0;

    headers.forEach((h) => {
      if (!sizing[h.column.id]) {
        sizing[h.column.id] = autoWidth;
      }
    });
  ...
  /**
   * Get first visible highlighted row index
   */
  ...
  /**
   * Capture actual rendered DOM widths before drag starts.
   * Wrapped in useCallback to avoid recreation on every render.
   */
  const captureColumnWidths = useCallback(() => {
    const sizing: ColumnSizingState = {};
    tableHeaderRef.current?.querySelectorAll('tr:first-child th').forEach((th, i) => {
      const hdr = table.getHeaderGroups()[0]?.headers[i];
      if (hdr) {
        sizing[hdr.column.id] = (th as HTMLElement).getBoundingClientRect().width;
      }
    });
    setColumnSizing(sizing);
  }, [table, tableHeaderRef]);
                    ...
                    >
                      ...
                      <div
                        className={cx(styles.resizeHandle, {
                          [styles.resizeHandleActive]: header.column.getIsResizing(),
                        })}
                        onMouseDown={(e) => {
                          // Capture handler BEFORE flushSync to avoid stale closure
                          const resizeHandler = header.getResizeHandler();
                          flushSync(() => {
                            captureColumnWidths();
                          });
                          resizeHandler(e as unknown as MouseEvent);
                        }}
                        onTouchStart={(e) => {
                          // Capture handler BEFORE flushSync to avoid stale closure
                          const resizeHandler = header.getResizeHandler();
                          ...
                          resizeHandler(e as unknown as TouchEvent);
                        }}
                      />
                    </th>
                  );
