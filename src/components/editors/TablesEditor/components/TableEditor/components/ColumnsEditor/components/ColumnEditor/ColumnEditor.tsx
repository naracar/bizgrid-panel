...
export const ColumnEditor: React.FC<Props> = ({ value, onChange, data, isAggregationAvailable, showTableHeader }) => {
  ...
  const [showInspector, setShowInspector] = useState(value.showInspector ?? true);
      ...
      <FieldsGroup label="Format">
        ...
        {value.type === CellType.JSON && (
          <>
            ...
            <InlineField label="Show Inspector" grow={true} tooltip="Show or hide the eye icon inspector">
              <InlineSwitch
                value={showInspector}
                onChange={(e) => {
                  const checked = e.currentTarget.checked;
                  setShowInspector(checked);
                  onChange({ ...value, showInspector: checked });
                }}
              />
            </InlineField>
          </>
